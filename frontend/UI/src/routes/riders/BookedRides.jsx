...
const BookedRides = () => {
    ...
    const [showDropMenu, setShowDropMenu] = useState(null);
    const [rideObj, setRide] = useState(null);
    ...
    const openRideInfo = (obj) => {
        setRide(obj);
        setShowDropMenu((prev) => (prev === obj.id ? null : obj.id)); // Toggle menu for selected ride
    };
    ...
    const cancelRide = (rideID) => {
        ...
        setShowDropMenu(null); // Close menu after action
    };

    return (
        <div className="w-full sm:w-[90%] mx-auto sm:pt-16">
            <h1 className="text-center mb-8 sm:text-xl font-semibold hover:text-deep-purple">
                List of Rides
            </h1>
            <div className="w-full flex gap-2 flex-wrap">
                                    ...
                                    <tbody>
                                        {rides
                                            ?.filter(
                                                (r) =>
                                                    r?.rider?.id ===
                                                    userDecodedToken?.riderProfileID
                                            )
                                            .map((ride) => (
                                                <tr className="border-b" key={ride?.id}>
                                                    ...
                                                    <td className="px-4 py-3">
                                                        {ride?.driver?.user?.first_name?.substring(
                                                            0,
                                                            10
                                                        ) || "No driver..."}
                                                    </td>
                                                    <td className="px-4 py-3">
                                                        {ride?.pick_up_location?.substring(0, 10)}
                                                    </td>
                                                    <td className="px-4 py-3">
                                                        {ride?.drop_off_location?.substring(0, 10)}
                                                    </td>
                                                    ...
                                                    <td className="px-4 py-3">
                                                        <span
                                                            className={`bg-green-800 font-semibold text-white p-1 rounded-sm ${
                                                    ...
                                                    <td className="px-4 py-3 relative">
                                                        ...
                                                        <div
                                                            className={`z-40 left-0 absolute bg-white divide-y divide-gray-100 rounded-lg shadow w-full ${
                                                                showDropMenu === ride.id
                                                                    ? "block"
                                                                    : "hidden"
                                                            }`}
                                                        >
                ...
                <div className={`flex-1 w-full`}>
                    {rideObj && <MapComp rideObj={rideObj} apiKey={mapboxApiKey} />}
                </div>
            </div>
        </div>
    );
